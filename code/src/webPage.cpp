...
void setupWebServer(){
  ...
  ESPWebServer.on(F("/")                 ,[](){ handleRoot(); onWebServerRequest();});
  ESPWebServer.on(F("/status")           ,[](){ configDeviceFromJS(); sendDeviceStatusToJS(); onWebServerRequest();});
...
function addTheDelay(i){var v,d,ret=document.createElement('div');\n\
 ...
 d.appendChild(v=document.createElement('span'));v.innerHTML='&nbsp;&nbsp;&nbsp;(Next state:&nbsp;';\n\
 d.appendChild(v=document.createElement('input'));v.type='checkbox';v.id='switch'+i+'-timer';v.classList.add('onofftimer');\n\
 d.appendChild(v=document.createElement('span'));v.innerHTML='&nbsp;at&nbsp;&nbsp;';\n\
